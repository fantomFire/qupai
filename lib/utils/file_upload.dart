...
class FileUpLoadServer {
  ...
  static Dio _dio;
  // 上传二维码
  static Future<HttpResponse> uploadzhifi(BuildContext context, File file, {bool showDialog: true, bool initState: false}) async {
    if (file != null && file.lengthSync() != 0) {
      if (!CommonUtil.checkImageType(file)) {
        ...
        try {
          ...
          Response response = await _dio.post( Urls.addAliPictuer, data: formData);
          ...
          HttpUtil.hideLoadingProgress(context, showDialog);
          if (e != null &&
              (e.type == DioErrorType.CONNECT_TIMEOUT || e.type == DioErrorType.RECEIVE_TIMEOUT || e.type == DioErrorType.SEND_TIMEOUT)) {
  ...
  // 上传图片
  static Future<HttpResponse> uploadImage(BuildContext context,String user_id, File file, {bool showDialog: true, bool initState: false}) async {
    if (file != null && file.lengthSync() != 0) {
      if (!CommonUtil.checkImageType(file)) {
        ToastUtil.toast("图片格式错误");
        return HttpResponse(false);
  ...
  static Future<HttpResponse> sendImage(BuildContext context,String user_id,String cs_id, File file, {bool showDialog: true, bool initState: false}) async {
    if (file != null && file.lengthSync() != 0) {
      if (!CommonUtil.checkImageType(file)) {
        ToastUtil.toast("图片格式错误");
        return HttpResponse(false);
  ...
  //上传视频
  static Future<HttpResponse> uploadvideo(BuildContext context, File file, {bool showDialog: true, bool initState: false,}) async {
